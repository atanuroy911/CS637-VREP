...
def get_contours(img: np.ndarray):
    ...
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # 1. cvtcolor
    # Process image using Gaussian blur.
    blur = cv2.GaussianBlur(gray, (5, 5), 0)  # gaussian blur
    # Process image using Color Threshold.
    _, thresh = cv2.threshold(blur, 60, 255, cv2.THRESH_BINARY)  # 3. Threshold
    # Erode and dilate to remove accidental line detections.
    mask = cv2.erode(thresh, None, iterations=2)  # masking -> Erode -> Dilate
    mask = cv2.dilate(mask, None, iterations=2)
    # Find the contours of the image.
    contours = cv2.findContours(mask.copy(), 1, cv2.CHAIN_APPROX_NONE)  # find contours -> Parameter (masking)
    # Use imutils to unpack contours.
    contours = imutils.grab_contours(contours)  # Unpack contours
    return contours
...
def follow_lane(img: np.ndarray):
    ...
    cv2.line(img, (cx, 0), (cx, 720), (255, 0, 0), 2)
    cv2.line(img, (0, cy), (1280, cy), (255, 0, 0), 2)
    cv2.drawContours(img, c, -1, (0, 255, 0), 2)

    # Calculate the angle we want to steer.
    angle = math.atan((135 - cx) / cy)
